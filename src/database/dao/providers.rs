...
impl Database {
    ...
    /// 判断指定 app 下是否存在自定义供应商。
    pub fn has_custom_provider(&self, app_type: &str) -> Result<bool, AppError> {
        let conn = lock_conn!(self.conn);
        let mut stmt = conn
            .prepare("SELECT id FROM providers WHERE app_type = ?1")
            .map_err(|e| AppError::Database(e.to_string()))?;
        ...
        while let Some(row) = rows.next().map_err(|e| AppError::Database(e.to_string()))? {
            let _id: String = row.get(0).map_err(|e| AppError::Database(e.to_string()))?;
            return Ok(true);
        }
        Ok(false)
    }
