...
pub fn get_config_json_path() -> PathBuf {
    get_proxy_dir().join("config.json")
}

/// 获取旧格式配置文件路径（向后兼容）
pub fn get_providers_json_path() -> PathBuf {
    get_proxy_dir().join("providers.json")
}

/// 获取配置文件路径（优先 config.json，回退 providers.json）
pub fn get_active_config_path() -> Option<PathBuf> {
    let config_path = get_config_json_path();
    if config_path.exists() {
        return Some(config_path);
    }
    let providers_path = get_providers_json_path();
    if providers_path.exists() {
        return Some(providers_path);
    }
    None
}
