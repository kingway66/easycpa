...
/// When true, skip the local settings store for current_provider lookups
/// and rely solely on the database's `is_current` flag.
/// Set during init when loading from providers.json.
static USE_JSON_CONFIG: AtomicBool = AtomicBool::new(false);

pub fn set_use_json_config(value: bool) {
    USE_JSON_CONFIG.store(value, Ordering::Relaxed);
}
...
pub fn get_effective_current_provider(
    ...
    // In JSON config mode, skip the settings store entirely —
    // the JSON file is the source of truth, not ~/.cc-switch/settings.json
    if USE_JSON_CONFIG.load(Ordering::Relaxed) {
        return db.get_current_provider(app_type.as_str());
    }
