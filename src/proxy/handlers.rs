    ...
    let models: Vec<Value> = routes
        ...
        .map(|r| {
            ...
            let levels: Vec<Value> = if r.supported_reasoning_levels.is_empty() {
                ...
                r.supported_reasoning_levels.iter().map(|l| {
                    json!({
                        "effort": l,
                        "description": format!("{} reasoning", l)
                    })
                }).collect()
            };
            ...
            let mut info = json!({
                "slug": r.name,
                "display_name": r.name,
                "description": format!("Proxy → {}", r.model),
            ...
            if let Some(cw) = r.context_window {
                info["context_window"] = json!(cw);
            }
            if let Some(max_out) = r.max_output_tokens {
                info["max_output_tokens"] = json!(max_out);
            }
            if let Some(ref level) = r.default_reasoning_level {
                info["default_reasoning_level"] = json!(level);
            }
            if let Some(parallel) = r.supports_parallel_tool_calls {
                info["supports_parallel_tool_calls"] = json!(parallel);
            }
            if let Some(summaries) = r.supports_reasoning_summaries {
                info["supports_reasoning_summaries"] = json!(summaries);
            }
...
///
/// 根据上游 provider 的 api_format 决定是否需要格式转换：
/// - openai_responses (默认): 透传
/// - openai_chat: Responses → Chat 请求转换 + Chat → Responses 响应转换
/// - anthropic: Responses → Anthropic → Chat 请求转换 + 反向响应转换
pub async fn handle_responses(
    ...
    log::info!(
        "[Codex] {} model={} format={} stream={} input_items={}",
        uri.path(),
    ...
    let needs_conversion = matches!(api_format, "openai_chat" | "anthropic");
    let (endpoint, forward_body) = if needs_conversion {
        let converted = match api_format {
            "openai_chat" => transform_responses_chat::responses_to_chat_request(body.clone())?,
            "anthropic" => {
                // Responses 请求 → Anthropic 请求（直接转换，发到 Anthropic 端点）
                transform_responses::responses_request_to_anthropic_request(body.clone())?
            }
            _ => body.clone(),
        };
        let ep = match api_format {
            "openai_chat" => endpoint_with_query(&uri, "/chat/completions"),
            "anthropic" => endpoint_with_query(&uri, "/v1/messages"),
            _ => endpoint_with_query(&uri, "/responses"),
        };
        log::debug!("[Codex] 转换后请求 (→ {}): {}", ep, serde_json::to_string_pretty(&converted).unwrap_or_default());
        (ep, converted)
    } else {
        (endpoint_with_query(&uri, "/responses"), body.clone())
    };
    ...
    if needs_conversion {
        // 需要响应转换
        handle_codex_responses_transform(response, &ctx, &state, &body, is_stream, api_format)
            .await
    } else {
        // 透传模式
...
pub async fn handle_responses_compact(
    ...
    let (endpoint, forward_body) = if needs_conversion {
        ...
        let ep = match api_format {
            "openai_chat" => endpoint_with_query(&uri, "/chat/completions"),
            "anthropic" => endpoint_with_query(&uri, "/v1/messages"),
            _ => endpoint_with_query(&uri, "/responses/compact"),
        };
        (ep, converted)
    } else {
        (endpoint_with_query(&uri, "/responses/compact"), body.clone())
    };
...
/// Codex Responses 格式转换处理
///
/// 当上游 api_format 不是 openai_responses 时，将 Chat/Anthropic 响应转回 Responses 格式
async fn handle_codex_responses_transform(
    response: super::hyper_client::ProxyResponse,
    ctx: &RequestContext,
    _state: &ProxyState,
    ...
    if is_stream || response.is_sse() {
        ...
        > = match api_format {
            ...
            "anthropic" => {
                // TODO: 缺少 Anthropic SSE → Responses SSE 流式转换器
                // 当前只支持非流式，流式会失败
                log::warn!("[Codex] anthropic 格式的流式响应暂不支持，建议在配置中使用 openai_chat");
                Box::new(Box::pin(create_responses_sse_stream_from_chat(stream)))
            }
            _ => {
                // Should not reach here (openai_responses is handled as passthrough)
                return Err(ProxyError::TransformError(format!(
                    "Unsupported api_format for Responses streaming transform: {api_format}"
                )));
            }
        };
        ...
        let timeout_config = ctx.streaming_timeout_config();
        let logged_stream =
            create_logged_passthrough_stream(sse_stream, "Codex/Responses", None, timeout_config);
    ...
    let upstream_response: Value = serde_json::from_slice(&body_bytes).map_err(|e| {
        log::error!("[Codex] 解析上游响应失败: {e}, body: {body_str}");
        ProxyError::TransformError(format!("Failed to parse upstream response: {e}"))
    })?;

    let responses_response = match api_format {
        "openai_chat" => {
            transform_responses_chat::chat_to_responses_response(upstream_response)?
        }
        "anthropic" => {
            // Anthropic 响应 → Responses 响应（直接转换）
            anthropic_response_to_responses(upstream_response)?
        }
        _ => {
            return Err(ProxyError::TransformError(format!(
                "Unsupported api_format for Responses transform: {api_format}"
            )));
        }
    };
    ...
    let response_body = serde_json::to_vec(&responses_response).map_err(|e| {
        log::error!("[Codex] 序列化响应失败: {e}");
        ProxyError::TransformError(format!("Failed to serialize response: {e}"))
    })?;

    let body = axum::body::Body::from(response_body);
    builder.body(body).map_err(|e| {
        log::error!("[Codex] 构建响应失败: {e}");
        ProxyError::Internal(format!("Failed to build response: {e}"))
    })
}

/// Anthropic 响应 → Responses 响应（简单转换）
fn anthropic_response_to_responses(body: Value) -> Result<Value, ProxyError> {
    let content = body
        .get("content")
    ...
    let status = match stop_reason {
        Some("end_turn") | Some("tool_use") => "completed",
        Some("max_tokens") => "incomplete",
        _ => "completed",
    };
    let incomplete_details = if stop_reason == Some("max_tokens") {
