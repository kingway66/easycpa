...
///
/// 将 Responses API 的请求格式转为 Anthropic Messages API 请求格式：
/// - instructions → system
/// - input[] → messages[]（message/function_call/function_call_output → role+content）
/// - max_output_tokens → max_tokens
/// - tools: Responses function → Anthropic tool
pub fn responses_request_to_anthropic_request(body: Value) -> Result<Value, ProxyError> {
    let mut result = json!({});
    ...
    if let Some(input) = body.get("input").and_then(|i| i.as_array()) {
        let messages = convert_responses_input_to_anthropic_messages(input)?;
    ...
    for item in input {
        ...
        match item_type {
            "message" => {
                ...
                let content_arr = item.get("content").and_then(|c| c.as_array());
                ...
                // 处理 reasoning item（在 message 之前）
                if let Some(reasoning) = pending_reasoning.take() {
                    // Anthropic 不支持在 messages 中嵌入 reasoning，跳过
                    let _ = reasoning;
                }
            ...
            "reasoning" => {
                // 收集 reasoning 内容
                let summary = item.get("summary").and_then(|s| s.as_array());
