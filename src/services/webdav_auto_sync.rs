...
fn log_auto_sync_status(status: &str, error: Option<&str>) {
    match error {
        Some(message) => log::info!("[WebDAV][AutoSync] status={status}, error={message}"),
        None => log::info!("[WebDAV][AutoSync] status={status}"),
    }
}
...
async fn run_auto_sync_upload(
    ...
    match result {
        Ok(_) => {
            log_auto_sync_status("success", None);
            Ok(())
        }
        Err(err) => {
            persist_auto_sync_error(&mut sync_settings, &err);
            log_auto_sync_status("error", Some(&err.to_string()));
            Err(err)
        }
    }
}
